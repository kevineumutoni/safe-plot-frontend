...
import { MapContainer, Marker, ZoomControl, useMapEvents, useMap } from 'react-leaflet'
...
function CustomLayerControl() {
  ...
  useEffect(() => {
    ...
    return () => { satRef.current?.remove(); mapRef.current?.remove() }
  }, [map])
    ...
    >
      {(['satellite', 'map'] as const).map(type => (
        <Button
          ...
          borderRadius="7px"
          border="none"
          bg={active === type ? '#2a9d5c' : 'transparent'}
          color={active === type ? 'white' : 'rgba(232,245,238,0.5)'}
          fontSize="11px"
          fontWeight={700}
          fontFamily="'Sora', sans-serif"
          transition="all 0.15s"
          _hover={{ bg: active === type ? '#22875a' : 'rgba(255,255,255,0.06)' }}
          height="auto"
        >
  ...
  const handleSelect = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (d?.detail === 'out_of_kigali' || e?.response?.status === 422) {
        ...
        setError(d?.detail ?? (lang === 'rw' ? 'Ikibazo cyabaye.' : 'Check failed — is backend running?'))
      }
      setPinColor('#6b7280')
    } finally {
      setChecking(false)
  ...
  return (
    <Box position="relative" height="100%" width="100%">
      ...
      <Box
        ...
        top={3}
        left={3}
        right={hasResult && onClose ? '200px' : '160px'}
        zIndex={1000}
      >
      ...
      {error && !checking && (
        ...
        >
          <Text fontSize="12px" color="#ef4444" fontWeight={600}>{error}</Text>
        </Box>
      )}
      ...
      >
        {[
          ...
          <HStack key={label} gap="7px">
            <Box
              width="12px" height="12px" borderRadius="3px"
              bg={bg} border={`1px solid ${bd}`} flexShrink={0}
            />
            <Text fontSize="10px" color="rgba(232,245,238,0.6)" fontWeight={600}>{label}</Text>
          </HStack>
        ))}
        <Box borderTop="1px solid rgba(255,255,255,0.06)" mt="2px" pt="4px">
          <Text fontSize="9px" color="rgba(232,245,238,0.3)" fontWeight={600}>
      ...
      <Box
        ...
        whiteSpace="nowrap"
      >
        <Text fontSize="11px" color="rgba(232,245,238,0.45)" letterSpacing="0.04em">
          {count === 0
